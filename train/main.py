...
first_network.load("2_nets_1_tanh")
...
for repeat in range(15):
    for sounds_dict in net_config["training"]["train_data"]:
        ...
        output = second_network.training_net(inputs=torch.FloatTensor(resizer.resize(lst=[i.detach().numpy()[0] for i in outputs],resize_to=16)),
                                    must_outputs=torch.FloatTensor(sounds_dict["must_output"]))
        print("Network think: ",make_result(output))
plt.plot(second_network.errors)
plt.show()
